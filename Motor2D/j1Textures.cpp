...
bool j1Textures::UnLoad(SDL_Texture* texture)
{
	...
	for(item = textures.begin(); item != textures.end(); item++)
	{
		...
		{
			SDL_DestroyTexture(item._Ptr->_Myval);
			textures.remove(item._Ptr->_Myval);
			return true;
		}
	}
...
SDL_Texture* const j1Textures::LoadSurface(SDL_Surface* surface)
{
	...
	else
	{
		...
		Size_texture* temp = new Size_texture();
		temp->height = surface->h;
		temp->width = surface->w;
		temp->texture = texture;
		size_textures.push_back(*temp);
	}
...
int j1Textures::GetWidth(const SDL_Texture* texture)
{
	std::list<Size_texture>::iterator item = size_textures.begin();
	int ret = 0;
	while (item != size_textures.end())
	{
...
int j1Textures::GetHeight(const SDL_Texture* texture)
{
	std::list<Size_texture>::iterator item = size_textures.begin();
	int ret = 0;
	while (item != size_textures.end())
